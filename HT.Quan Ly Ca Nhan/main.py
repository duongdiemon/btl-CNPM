...
from controllers import AuthController, TaskController, FinanceController, NoteController, ProfileController
...
def main():
    if not st.session_state.is_logged_in:
        ...
        if menu == "Quản lý công việc":
            tasks_page(task_controller)
        elif menu == "Quản lý thời gian":
            time_management_page()
        elif menu == "Quản lý tài chính":
            finance_page(finance_controller)
        elif menu == "📝 Ghi chú":
            notes_page(note_controller)
        elif menu == "📦 Sao lưu":
            backup_page()
        elif menu == "👤 Thông tin cá nhân":
            profile_page(profile_controller)
